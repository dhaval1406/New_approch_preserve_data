...
# New Category file
category <- fread('category.tsv', header = FALSE, sep = "\t", na.strings=c("NA", ''))
...
##### Import all monthly data
      ...
      fileNames <- list.files(path = getwd(), pattern = "codefix_log_*")
      codeFix_log <- rbindlist(lapply(fileNames, function(x) {
        xx <- fread(x, header = TRUE, sep = "\t", na.strings=c("NA", ''))
      }))
      
      codeFix_log <- unique(codeFix_log)

      s.file = 'search_log_20130601_20130901.txt'
      c.file = 'codefix_log_20130601_20130901.txt'
############################################################################
...
analysis_11_results <- search_codefix_so_merge[, list(sum_product_total_amount = sum(Product_Total_Amount, na.rm = T),
                                                      sales_count = length(Product_Name)
                                                      ), 
                                                by=list(cat_id_2, cat_name_2)
                                             ] [ order(-sum_product_total_amount, na.last=TRUE) ]

analysis_file = paste0("P:/Data_Analysis/Analysis_Results/11_SO_Data_cat2", "_", file.frm.date, "_", file.to.date, ".csv" )

# Exporting results to csv
# write.csv(analysis_11_results, file = "P:/Data_Analysis/Analysis_Results/keyword_analysis_6.csv", na = '', row.names = FALSE)
write.csv(analysis_11_results, file = analysis_file, na = '', row.names = FALSE)

unlink(c(s.file, c.file))

# save image of R data for further use
image_file = paste0("P:/Data_Analysis/Processed_R_Datasets/Processed_Data_11", "_", file.frm.date, "_", file.to.date, ".RData" )
